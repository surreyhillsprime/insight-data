...
def enrich_transactions(transactions, cache, args):
    ...
    context_fields = ("latitude", "longitude", "geocode", "openStreetMap")
    donors = {}
    ...
    for index, item in enumerate(transactions, start=1):
        output = dict(item)
        # Live Environment Agency status was retired from the public product.
        # Remove any legacy observation even during a missing-only alignment.
        output.pop("environmentAgency", None)
